...
public class ElectronicsDAOImpl implements ElectronicsDAO {
	...
	public List<Electronics> selectAll() throws SQLException {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				Electronics electonics = new Electronics(rs.getString(1), rs.getString(2), rs.getInt(3),
	...
	public List<Electronics> getBoardList(int pageNo) throws SQLException {
		...
		String sql = proFile.getProperty("query.pagingSelect");// select * from (SELECT a.*, ROWNUM rnum FROM (SELECT *
																// FROM Electronics ORDER BY writeday desc) a) where
																// rnum>=? and rnum <=?
		try {
			...
			int totalPage = totalCount % PageCnt.pagesize == 0 ? totalCount / PageCnt.pagesize
	...
	public Electronics selectByModelNum(String modelNum) throws SQLException {
		...
		try {
			...
			rs = ps.executeQuery();
			if (rs.next()) {
				electronics = new Electronics(rs.getString(1), rs.getString(2), rs.getInt(3), rs.getString(4),
	...
	public int increamentByReadnum(String modelNum) throws SQLException {
		...
		PreparedStatement ps = null;
		int result = 0;
		String sql = proFile.getProperty("query.updateReadnum");// update Electronics set readnum=readnum+1 where
																// model_num=?
		try {
			con = DbUtil.getConnection();
			ps = con.prepareStatement(sql);
	...
	public int update(Electronics electronics) throws SQLException {
		...
		PreparedStatement ps = null;
		int result = 0;
		String sql = proFile.getProperty("query.update");// update Electronics set model_name=?,price=?,description=?
	...
	@Override
	public List<Electronics> selectByAllModelNum(String modelName) throws SQLException {
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null; // select니까 있어야함

		List<Electronics> list = new ArrayList<>();
		String sql = "select * from Electronics where model_name like ?";
		try {
			con = DbUtil.getConnection();
			ps = con.prepareStatement(sql);
	...
	@Override
	public List<Electronics> selectByAllWriteUser(String writeUser) throws SQLException {
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null; // select니까 있어야함

		List<Electronics> list = new ArrayList<>();
		String sql = "select * from Electronics where upper(writeuser) like upper(?)";

		try {
			con = DbUtil.getConnection();
			ps = con.prepareStatement(sql);
			ps.setString(1, "%" + writeUser + "%");
			rs = ps.executeQuery();
